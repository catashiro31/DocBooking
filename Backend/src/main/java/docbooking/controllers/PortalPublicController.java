...
import docbooking.dtos.responses.DoctorCardResonseDTO;
import docbooking.dtos.responses.DoctorDetailResonseDTO;
import docbooking.dtos.responses.ReviewResponseDTO;
...
public class PortalPublicController {
    ...
    public ResponseEntity<?> getDoctors(
        ...
        List <DoctorCardResonseDTO> response = portalService.getDoctors(keyword, specId, minPrice, priceTo);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/doctors/{id}")
    public ResponseEntity<?> getDoctorDetail(@PathVariable Integer id) {
        DoctorDetailResonseDTO response = portalService.getDoctorById(id);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/doctors/{id}/reviews")
    public ResponseEntity<?> getDoctorReviews(@PathVariable Integer id){
        List<ReviewResponseDTO> response = portalService.getReviewsByDoctorId(id);
        return ResponseEntity.ok(response);
    }
